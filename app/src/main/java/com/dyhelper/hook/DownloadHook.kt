...
" + url else url
    ...
    fun downloadVideo(ctx: Context) { download(ctx, capture.getVideoUrl(), ".mp4") }
    fun downloadAudio(ctx: Context) { download(ctx, capture.getMusicUrl(), ".mp3") }
    fun downloadImage(ctx: Context) { download(ctx, null, ".jpg") }

    private fun download(ctx: Context, url: String?, ext: String) {
        if (url == null) {
            Toast.makeText(ctx, "No URL", Toast.LENGTH_SHORT).show()
            return
        }
