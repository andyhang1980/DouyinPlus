...
class ShareMenuHook(
    ...
    override fun init(loader: ClassLoader): Boolean {
        ...
        for (m in cls.declaredMethods) {
            if (m.name == "onMeasure" && m.parameterTypes.size == 2 &&
                ...
                HookUtils.log("[Menu] Hooked WrapSizeLinearLayout.onMeasure")
                return true
            }
        }
        return false
    ...
    private fun addMenu(container: ViewGroup, ctx: Context) {
        ...
        items.add(MI("Copy", Runnable { copyFn(ctx) }))
        items.add(MI(if (img) "Image" else "Video", Runnable {
            if (img) imageFn(ctx) else videoFn(ctx)
        }))
        items.add(MI("Audio", Runnable { audioFn(ctx) }))
